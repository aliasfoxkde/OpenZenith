...
describe("Elevation Batch API", () => {
  it("returns elevations for valid points", async () => {
    ...
    const req = new Request("http://localhost/api/elevation/batch", {
      method: "POST",
      body: JSON.stringify({ points: [{ lat: 40.7, lon: -74.0 }, { lat: 51.5, lon: -0.1 }] }),
    });
  ...
  it("rejects invalid coordinates", async () => {
    ...
    const req = new Request("http://localhost/api/elevation/batch", {
      method: "POST",
      body: JSON.stringify({ points: [{ lat: 999, lon: 0 }] }),
    });
    const resp = await POST(req);
    expect(resp.status).toBe(400);
  });
  ...
  it("preserves optional id field", async () => {
    ...
    const req = new Request("http://localhost/api/elevation/batch", {
      method: "POST",
      body: JSON.stringify({ points: [{ lat: 40.7, lon: -74.0, id: "nyc" }] }),
    });
    const resp = await POST(req);
    const data = await resp.json();
