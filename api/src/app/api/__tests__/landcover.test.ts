...
describe("Landcover Tile API", () => {
  it("proxies CORINE land cover WMS tiles", async () => {
    ...
    const resp = await GET(new Request("http://localhost/api/landcover/5/15/10"), { params: Promise.resolve({ z: "5", x: "15", y: "10" }) });
  ...
  it("returns error for out of range zoom", async () => {
    const { GET } = await import("@/app/api/landcover/[z]/[x]/[y]/route");
    const resp = await GET(new Request("http://localhost/api/landcover/0/0/0"), { params: Promise.resolve({ z: "0", x: "0", y: "0" }) });
