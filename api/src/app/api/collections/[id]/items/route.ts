...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(source.url.startsWith("/") ? `${baseUrl}${source.url}` : source.url, {
    ...
    const links = [
      {
        ...
        href: `${baseUrl}/api/collections/${id}/items?limit=${limit}&offset=${offset}`,
      },
      { rel: "collection", type: "application/json", href: `${baseUrl}/api/collections/${id}` },
    ...
    if (hasNext) {
      links.push({
        rel: "next",
        type: "application/geo+json",
        href: `${baseUrl}/api/collections/${id}/items?limit=${limit}&offset=${offset + limit}`,
      });
    }
    if (offset > 0) {
      links.push({
        rel: "prev",
        type: "application/geo+json",
        href: `${baseUrl}/api/collections/${id}/items?limit=${limit}&offset=${Math.max(0, offset - limit)}`,
      });
    }
