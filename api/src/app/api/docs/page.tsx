...
interface RequestBody {
  ...
  content: {
    "application/json": {
      schema: { type: string; required?: string[]; properties?: Record<string, Record<string, unknown>>; description?: string; maxLength?: number };
  ...
  return (
    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
      <thead>
        <tr>
          {["Name", "In", "Type", "Required", "Description"].map((h) => (
            <th key={h} style={{
              textAlign: "left", padding: "0.5rem 0.75rem", borderBottom: "1px solid #333",
              color: textDim, fontWeight: 500, fontSize: "0.75rem", textTransform: "uppercase", letterSpacing: "0.05em",
            }}>{h}</th>
          ))}
        </tr>
      </thead>
      <tbody>
        {params.map((p) => (
          <tr key={p.name}>
            <td style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid #1a1a1a", fontFamily: "monospace", color: accent, fontSize: "0.85rem" }}>
              {p.name}
            </td>
            <td style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid #1a1a1a", color: textDim }}>
              {p.in}
            </td>
            <td style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid #1a1a1a", color: textDim, fontSize: "0.8rem" }}>
              {p.schema?.type}
            ...
            <td style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid #1a1a1a" }}>
              {p.required ? (
                <span style={{ background: "#3b1c1c", color: "#f87171", padding: "0.15rem 0.5rem", borderRadius: 4, fontSize: "0.75rem" }}>required</span>
              ) : (
                <span style={{ background: "#1a2e1a", color: "#4ade80", padding: "0.15rem 0.5rem", borderRadius: 4, fontSize: "0.75rem" }}>optional</span>
              )}
            </td>
            <td style={{ padding: "0.5rem 0.75rem", borderBottom: "1px solid #1a1a1a", color: "#ccc", lineHeight: 1.4 }}>
              {p.description}
              {p.example !== undefined && (
...
function EndpointCard({ path, method, endpoint, serverUrl }: {
  path: string; method: string; endpoint: Endpoint; serverUrl: string;
}) {
    ...
    <div style={{ border: `1px solid ${border}`, borderRadius: 12, overflow: "hidden", marginBottom: "0.75rem" }}>
      <button
        ...
        style={{
          ...
          padding: "1rem 1.25rem", background: "#111", border: "none",
          cursor: "pointer", textAlign: "left", color: text, fontFamily: "inherit",
        }}
      >
        <span style={{
          background: color, color: "#000", fontWeight: 700, fontSize: "0.7rem",
          padding: "0.2rem 0.5rem", borderRadius: 4, minWidth: 48, textAlign: "center", textTransform: "uppercase",
        }}>
          {method}
        </span>
        ...
        <div style={{ padding: "1.25rem", background: "#0d0d0d" }}>
          ...
          {endpoint.parameters && endpoint.parameters.length > 0 && (
            <div style={{ marginBottom: "1.25rem" }}>
              <h4 style={{ margin: "0 0 0.75rem", fontSize: "0.8rem", color: textDim, textTransform: "uppercase", letterSpacing: "0.05em" }}>
                Parameters
              </h4>
          ...
          {endpoint.requestBody && (
            <div style={{ marginBottom: "1.25rem" }}>
              ...
              <textarea
                ...
                rows={5}
                style={{
                  width: "100%", background: "#1a1a1a", color: "#e5e5e5", border: `1px solid #333`,
                  borderRadius: 8, padding: "0.75rem", fontFamily: "'SF Mono', 'Fira Code', monospace",
                  fontSize: "0.8rem", lineHeight: 1.5, resize: "vertical", outline: "none", boxSizing: "border-box",
                }}
              />
            </div>
          )}
          ...
          <div style={{ marginBottom: "1.25rem" }}>
            ...
            {Object.entries(endpoint.responses).map(([code, resp]) => (
              <div key={code} style={{ marginBottom: "0.75rem" }}>
                ...
                {resp.content && Object.entries(resp.content).map(([ct, body]) => (
                  <div key={ct} style={{ marginBottom: "0.5rem" }}>
                    {body.schema && (
                      <div style={{ fontFamily: "monospace", fontSize: "0.75rem", color: "#666", marginBottom: "0.3rem" }}>
                        Content-Type: {ct}
                      </div>
                    )}
          ...
          <div>
              ...
              <div style={{ marginBottom: "0.75rem", display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                {endpoint.parameters.filter((p) => p.in === "query").map((p) => (
                  <div key={p.name} style={{ flex: "1 1 180px" }}>
                    <label style={{ display: "block", fontSize: "0.7rem", color: textDim, marginBottom: "0.2rem", fontFamily: "monospace" }}>
                      {p.name}{p.required ? " *" : ""}
                    </label>
                    <input
                      type="text"
                      ...
                      style={{
                        width: "100%", background: "#1a1a1a", color: "#e5e5e5", border: `1px solid #333`,
                        borderRadius: 6, padding: "0.4rem 0.6rem", fontSize: "0.8rem", fontFamily: "monospace",
            ...
            {/* URL preview */}
            <div style={{
              background: "#1a1a1a", borderRadius: 6, padding: "0.5rem 0.75rem",
              fontFamily: "monospace", fontSize: "0.75rem", color: "#555", marginBottom: "0.75rem",
            ...
            <button
              ...
              disabled={loading}
              style={{
                background: color, color: "#000", border: "none", padding: "0.5rem 1rem",
                borderRadius: 6, fontSize: "0.85rem", fontWeight: 600, cursor: loading ? "wait" : "pointer", fontFamily: "inherit",
              }}
            >
  ...
  if (!spec) {
    return (
      <div style={{
        ...
        alignItems: "center", justifyContent: "center", color: textDim, fontFamily: "inherit",
      }}>
        Loading docs...
      </div>
    );
    ...
    <div style={{ minHeight: "100vh", background: bg, color: text, fontFamily: "inherit" }}>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 1.5rem 2rem" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
          <span style={{ background: "#1a2e1a", color: accent, padding: "0.2rem 0.6rem", borderRadius: 4, fontSize: "0.8rem", fontFamily: "monospace" }}>
            v{spec.info.version}
          </span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "0.75rem" }}>
          <a href="/map" style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
              <div style={{
                width: 28, height: 28, borderRadius: 6, background: "rgba(34,197,94,0.12)",
                display: "flex", alignItems: "center", justifyContent: "center", color: accent, fontSize: "0.9rem",
              }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2C8.13 2 5 4.13 5 8c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7z" /><circle cx="12" cy="10" r="3" /></svg>
              </div>
            ...
            <p style={{ margin: "0 0 0.5rem", fontSize: "0.8rem", color: textDim, lineHeight: 1.5 }}>
              Click any point for elevation. Hillshade, 3D terrain, contour lines, 6 basemaps, elevation pins, and profile tool.
            </p>
          ...
          <a href="/worldview" style={{
            ...
            <div style={{ fontSize: "0.7rem", color: "#555", fontFamily: "monospace", lineHeight: 1.6 }}>
              Layers: earthquakes, radar, flights, satellites,<br />
          ...
          <a href="/explore" style={{
            ...
            <p style={{ margin: "0 0 0.5rem", fontSize: "0.8rem", color: textDim, lineHeight: 1.5 }}>
              Discover ArcGIS REST services and query OpenStreetMap via Overpass API. Browse layers, preview features, run custom queries.
            </p>
            <div style={{ fontSize: "0.7rem", color: "#555", fontFamily: "monospace", lineHeight: 1.6 }}>
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          <button
            ...
            style={{
              ...
              cursor: "pointer", fontFamily: "inherit", fontWeight: !activeTag ? 600 : 400,
            }}
          >
            All
          </button>
          {spec.tags.map((tag) => (
            <button
              ...
              style={{
                ...
                padding: "0.35rem 0.75rem", borderRadius: 6, fontSize: "0.8rem",
                cursor: "pointer", fontFamily: "inherit", fontWeight: activeTag === tag.name ? 600 : 400,
              }}
            >
              {tag.name}
            </button>
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "0 1.5rem 3rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          {CODE_EXAMPLES.map((section) => (
            <div key={section.title} style={{ background: cardBg, border: `1px solid ${border}`, borderRadius: 12, padding: "1.25rem" }}>
              ...
              {section.examples.map((ex) => (
                <div key={ex.lang} style={{ marginBottom: "0.5rem" }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: "0.7rem", color: textDim, textTransform: "uppercase", fontWeight: 600, letterSpacing: "0.05em" }}>
                      {ex.lang}
                    </span>
                    <CopyBtn text={ex.code} label={`Copy ${ex.lang}`} />
                  </div>
                  <pre style={{
                    background: "#1a1a1a", color: "#e5e5e5", padding: "0.8rem 1rem",
                    borderRadius: 8, fontSize: "0.78rem", lineHeight: 1.6, overflowX: "auto",
