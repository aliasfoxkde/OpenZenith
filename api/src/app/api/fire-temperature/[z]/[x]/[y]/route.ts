...
const LAYER = "GOES-East_ABI_FireTemp";
...
export async function GET(request: Request, { params }: { params: Promise<{ z: string; x: string; y: string }> }) {
  ...
  // Try R2 cache first
  const cached = await r2GetTile("fire-temperature", zoom, tileX, tileY);
  if (cached) {
    return new Response(cached, {
      headers: { "Content-Type": "image/png", "Cache-Control": "public, max-age=3600", "X-Cache": "HIT", ...CORS_HEADERS },
    });
  }
  ...
  try {
    ...
    r2PutTile("fire-temperature", zoom, tileX, tileY, buffer, contentType).catch(() => {});

    return new Response(buffer, {
      headers: { "Content-Type": contentType, "Cache-Control": "public, max-age=3600", "X-Cache": "MISS", ...CORS_HEADERS },
    });
  } catch {
