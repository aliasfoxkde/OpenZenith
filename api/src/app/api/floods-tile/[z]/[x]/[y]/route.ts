...
const LAYER = "VIIRS_Combined_Flood_3-Day";
...
export async function GET(request: Request, { params }: { params: Promise<{ z: string; x: string; y: string }> }) {
  ...
  if (isNaN(zoom) || isNaN(tileX) || isNaN(tileY) || zoom < 0 || zoom > 9) {
  ...
  // Try R2 cache first
  const cached = await r2GetTile("floods-tile", zoom, tileX, tileY);
  if (cached) {
  ...
  try {
    ...
    r2PutTile("floods-tile", zoom, tileX, tileY, buffer, contentType).catch(() => {});
