...
export function addSeaHeight(map: maplibregl.Map, handle: LayerHandle): void {
  ...
  try {
    map.addSource("sea-height", { type: "raster", tiles: ["/api/sea-height/{z}/{x}/{y}"], tileSize: 256, minzoom: 0, maxzoom: 6 });
  ...
  } catch { setStatus(handle, "seaHeight", "error"); }
}
export function removeSeaHeight(map: maplibregl.Map): void {
  try { map.removeLayer("sea-height-raster"); } catch {}
  try { map.removeSource("sea-height"); } catch {}
}
