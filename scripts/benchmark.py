...
from openzenith.tile_format import (
    encode, decode, COMP_ZSTD, COMP_ZSTD_DELTA, COMP_ZSTD_PREDICT,
)
from openzenith.geo_utils import (
    load_geotiff, compute_rmse, compute_slope_deviation,
    classify_terrain, srtm_filename_to_bounds,
)
...
def run_benchmark(output_dir, tiles=None, max_tiles=None):
    ...
    for terrain_label, fname in tile_list.items():
        ...
        print(f"  Shape: {arr.shape}, File: {src_size/1e6:.1f}MB, "
              f"Elev: [{arr[valid].min()}..{arr[valid].max()}]m")
        ...
        for strategy in STRATEGIES:
            ...
            try:
                if method in ("gzip", "bz2", "lzma", "zstd"):
                    ...
                    comp_mode = method
                    t0 = time.time()
                    encoded = encode(arr, compression=comp_mode, zstd_level=level,
                                     quantize_bits=qbits, nodata_value=nodata)
        ...
        for r in tile_results:
            ...
            print(f"  {r['strategy']:<22} {r['compressed_bytes']/1024:>8.1f}K "
                  f"{r['ratio']:>5.1f}x {r['reduction_pct']:>6.1f}% "
                  f"{r['rmse']:>7.3f}m {r['encode_time_s']:>5.2f}s {r['decode_time_s']:>5.2f}s {typ:>10}")

        all_results.append({
            "tile": fname, "terrain_label": terrain_label,
            "terrain_type": terrain, "bounds": bounds,
            "source_bytes": src_size, "shape": list(arr.shape),
...
def _print_summary(all_results, output_dir):
    ...
    for tile in all_results:
        for r in tile["strategies"]:
            ...
            if name not in strategy_stats:
                strategy_stats[name] = {
                    "count": 0, "total_source": 0, "total_compressed": 0,
                    "rmse_sum": 0, "max_rmse": 0, "slope_sum": 0,
                    "enc_sum": 0, "dec_sum": 0, "lossless_count": 0,
                }
            s = strategy_stats[name]
            s["count"] += 1
            s["total_source"] += r["source_bytes"] if "source_bytes" in r else 0
    ...
    for s in summary:
        ...
        print(f"{s['strategy']:<22} {s['tiles']:>5} {s['avg_ratio']:>5.1f}x "
    ...
    if nl:
        ...
        print(f"  NEAR-LOSSLESS (<1m): {best_nl['strategy']} - {best_nl['avg_reduction_pct']}% reduction, "
    ...
    if ag:
        ...
        print(f"  BALANCED (1-6m): {best_ag['strategy']} - {best_ag['avg_reduction_pct']}% reduction, "
    ...
    if vis:
        ...
        print(f"  VISUAL (6m+): {best_vis['strategy']} - {best_vis['avg_reduction_pct']}% reduction, "
    ...
    if lossless:
        best = best_ll
        avg_out = sum(r["compressed_bytes"] for t in all_results for r in t["strategies"]
                       if r["strategy"] == best["strategy"] and "error" not in r) / max(best["tiles"], 1)
